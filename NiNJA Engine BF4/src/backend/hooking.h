...
public:
	...
	template <typename T>
	bool HookFunction(DWORD dwAddress, void* pHookFunction, T* pTrampoline, const char* name = nullptr) {
		if (dwAddress > 0x80000000) {
			...
			HookContext context;
			context.dwAddress = dwAddress;
			memcpy(context.szAsm, (void*)dwAddress, 0x10);
			vHookContext.push_back(context);

			HookFunctionStart((DWORD*)dwAddress, startStub, (DWORD)pHookFunction);

			if (!pTrampoline) { 
				DbgPrint("Failed to hook %s", name);
				return false; 
			}

			*pTrampoline = (T)startStub;

			DbgPrint("Hooked: %s - Address=0x%X Stub=0x%X", name, dwAddress, *pTrampoline);
			return true;
		}

		DbgPrint("Failed to hook 0x%X", dwAddress);
	...
	bool HookFunctionNoLink(DWORD dwAddress, void* pHookFunction) {
		if (dwAddress > 0x80000000) {
			...
			DbgPrint("Hooked: Address=0x%X", dwAddress);
			return true;
		}
...
public:
	...
	void Branch(DWORD dwFrom, DWORD dwTo, bool linked = false);
...
public:
	...
	virtual void SetupDetour(DWORD dwModuleBaseAddr, char* szImportedModuleName, int Ordinal, PVOID Destination) {
		HANDLE hModule;
		XexPcToFileHeader((PVOID)dwModuleBaseAddr, (PLDR_DATA_TABLE_ENTRY*)&hModule);
		SetupDetour(hModule, szImportedModuleName, Ordinal, Destination);
	}

	virtual void SetupDetour(HANDLE hModule, char* szImportedModuleName, int Ordinal, PVOID Destination) {
		DWORD dwAddress;
